...
<body id="main" onload="setProductCategory('${product.category}')"><div class="content">

<div>
    <c:url value="${path}/product/adminProductList.do" var="backurl"/>
    <div class="loc icon"><samp class="t12"></samp>当前位置：商品管理&nbsp;&raquo;&nbsp;<a href="${path}/product/adminProductList.do" title="商品列表">商品列表</a>&nbsp;&raquo;&nbsp;<span class="gray">添加或编辑商品</span>
        <a href="${path}/product/adminProductList.do" title="返回商品列表" class="inb btn80x20">返回商品列表</a>
    </div>
    <form id="form111" name="form111" action="${path }/product/adminProductSaveEdit.do" method="post"> <%--enctype="multipart/form-data"--%>
        <div class="edit set">
            <input type="hidden" name="id" value="${product.id}"/>
            <p><label><samp>*</samp>商品名称：</label><input type="text" id="name" name="name" value="${product.name}" class="text state" reg2="^[a-zA-Z0-9\u4e00-\u9fa5]{1,20}$" tip="必须是中英文或数字字符，长度1-20"/>
                <span></span>
            </p>
            ...
            <p><label class="alg_t"><samp>*</samp>商品图片：</label><img id="imgSrc" src="${path}${product.imgUrl}" height="100" width="100" />
            </p>
            <p><label></label><input type='file' size='27' id='upload' name='upload' class="file" onchange="submitUpload()" /><span id="submitImgTip" class="pos">请上传图片宽为120px，高为50px，大小不超过100K。</span>
                <input type='hidden' id='img' name='imgUrl' value=''/><span></span>
            </p>

            <p><label><samp>*</samp>商品价格：</label><input type="text" name="price" value="${product.price}" class="text state" maxlength="100"  reg2="^([1-9][\d]{0,7}|0)(\.[\d]{1,2})$" tip="输入错误！"/>
                <span class="pos">&nbsp;</span>
            </p>
            <p><label><samp>*</samp>商品数量：</label><input type="number" name="pnum" value="${product.pnum}" class="text state" maxlength="100"  reg2="^\d+(\.\d+)?$" tip="输入错误！"/>
                <span class="pos">&nbsp;</span>
            </p>
            <p><label class="alg_t">商品描述：</label><textarea rows="4" cols="45" name="description" class="are" reg1="^(.|\n){0,300}$" tip="任意字符，长度0-300">${product.description}</textarea>
                <span class="pos">&nbsp;</span>
            </p>
            <p><label>是否显示：</label>
                <select id="display" name="display">
                    <option value="是" ${product.display=="是" ? 'selected="selected"' : ""}>是</option>
                    <option value="否" ${product.display=="否" ? 'selected="selected"' : ""} >否</option>
                </select>
            </p>
