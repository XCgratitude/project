...
<div class="frameR"><div class="box"><div class="content">
    ...
    <form action="${path}/order/listGuestOrder.do" id="form1" name="form1" method="post">
        ...
        <span class="r page inb_a">
            ...
            <c:if test='${pagination.pageNo==1}'><span class="inb" title="上一页"></span></c:if>
            <c:if test='${pagination.pageNo>1}'><a href="javascript: gotoPage('${pagination.prePage}');" title="上一页"></a></c:if>
            <c:if test='${pagination.totalPage==pagination.pageNo}'><span class="inb" title="下一页"></span></c:if>
            <c:if test='${pagination.totalPage>pagination.pageNo}'><a href="#" onclick="gotoPage('${pagination.nextPage}')"></a></c:if>
            <input type="text" name="pageNo" id="pageNo" class="txts" value="${pagination.pageNo}" size="7"  />
            <input type="button" onclick="gotoPage('jump')" value="" class="hand" />
        </span>
        </div>
    </form>
